...
## that w allows you to write to a file 
...
When you write to a file, Python may use a buffer to 
temporarily store the data before actually writing it to the disk. 
Closing a file forces the buffer to flush, meaning all data is 
physically written to the file, ensuring data integrity.

It ensures the integrity of the data when you close.
...
'''
Exercise
Create a program that reads a text file named "example.txt" 
...
The file contains the following columns: Product Name, Date Sold, 
Units Sold, Price per Unit, Total Sale
Write a Python script that reads the data from the CSV file and 
calculates the total revenue generated by each product.
Make sure to ignore the first line, since that's the header.
You can go through the file line by line, and split each line 
into strings based on commas to get the individual column values.
...
#       output = next(data) # removes our header
...
# result = df.set_index("Product Name")[Total]
...
'''
Exercise 
Create a program that asks the user for their name and favorite color, 
then writes this information to a new text file named "user_info.txt".
Write the name on the first line of the file, and the age on the 
